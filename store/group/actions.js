...
export async function load_provider(
  ...
  return new Promise(async (resolve, reject) => {
    try {
      let response = await this.$axios.get('/api/v2/group/' + id + '/provider', {
        params: {
          ...pagination,
        },
      })
...
export async function update_provider({ commit }, { group_id, provider_id, data }) {
  return new Promise(async (resolve, reject) => {
    try {
      let response = await this.$axios.put(`/api/v2/group/${group_id}/provider/${provider_id}`, {
        ...data,
      })
      resolve(response)
    } catch (error) {
...
export async function delete_provider({ commit }, { id, provider_id }) {
  return new Promise(async (resolve, reject) => {
    try {
      let response = await this.$axios.delete(`/api/v2/group/${id}/provider/${provider_id}`)
      resolve(response)
    } catch (error) {
      reject(error)
    }
...
export async function arrange_provider({ commit }, { id, provider_id, position }) {
  return new Promise(async (resolve, reject) => {
    try {
      let response = await this.$axios.put(`/api/v2/group/${id}/provider/${provider_id}/arrange`, {
        position,
      })
      resolve(response)
    } catch (error) {
...
export async function delete_type({ commit }, { id, type_id }) {
  return new Promise(async (resolve, reject) => {
    try {
      let response = await this.$axios.delete(`/api/v2/group/${id}/type/${type_id}`)
      resolve(response)
    } catch (error) {
      reject(error)
    }
